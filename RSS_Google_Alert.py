...
ref_table = 'ebola'                                                             #Reference table structure for all subsequent tables
home_path = os.getenv('HOME') + '/'
DB_mgr_feed = db_class_general.DBManager(sqlite3.connect(home_path+'/PycharmProjects/Aggregator/News_Db.db'))


def update_feed(rss_url,modified):
    feed = feedparser.parse(rss_url,modified)            #Ebola
...
def get_site_title_from_URL(url,query_flag=True):
    ...
    #Using Google Custom Search API. But limits it to 100 queries a day
    api_key = 'AIzaSyBrXC9mQcRC4vdTjYeSzdtSVdDOG6LFJZg'
    search_engine_id  = '015193181658669744808:1jdtotslm4i'
...
######################################### GENERATING AND FORMATTING JSON FOR GMAP JAVASCRIPT ####################################
'''    if not place_names_list:
        return None
    else:

        mentions = {"mentions":[]}
        article_location_template = {"article_location":[]}
        article_title_template = {"article_title":[]}
        article_link_template = {"article_url":[]}
        for ind,item in enumerate(location):
            latlng = get_latlng(item)
            article_location_template["article_location"].append({"lat":latlng[0],"lng":latlng[1]})
            article_title_template["article_title"].append(Title[ind])
            article_link_template["article_url"].append(Link[ind])


        for ind,item in enumerate(place_names_list):
            mentions['mentions'].append({'name':item,'lat':place_latlng[ind][0],'lng':place_latlng[ind][1]})


        out = dict(article_link_template.items() + article_location_template.items() + article_title_template.items() + mentions.items())
        with open(home_path + 'PycharmProjects/Aggregator/JS_Files/data.json','w') as outputfile:
...
'''
markers = []
key = 'AIzaSyBrXC9mQcRC4vdTjYeSzdtSVdDOG6LFJZg'
base_url = 'https://maps.googleapis.com/maps/api/staticmap?'
string_list = list(string.ascii_uppercase)
for ind,item in enumerate(place_names_list):
    markers.append('markers=color:green|label:%s|%s' %(string_list[ind],item))

markers = '&'.join(markers)

return base_url + markers + '&size=640x640' '&key=' + key
...
def main():
    Google_Alerts_url = 'https://www.google.com/alerts/feeds/15656279626899631164/17887603782746256536'           #Ebola
    Google_news_url = 'https://www.google.com/news?hl=en&gl=in&tbm=nws&as_drrb=q&as_qdr=a&q=ebola&oq=ebola&num=100&output=rss'
    ...
    if feed.status == 200 or feed.status==301:
        ...
        for item in feed['entries']:
            ...
            time_str = (timestamp + datetime.timedelta(hours=5.5)).strftime('%Y-%m-%d : %H:%M:%S')
...
main()
analyze_links_retrospectively(table_name='ebola',sentiment=1,start_id=1)
...
#webbrowser.get('/usr/bin/google-chrome %s').open(full_url.replace('|','%7C'))
